...
parser.add_argument(
    ...
    "--img_path",
    type=str,
    default="data/original_medsam_data/train/images",
...
parser.add_argument(
    ...
    "--gt_path",
    type=str,
    default="data/original_medsam_data/train/labels",
...
parser.add_argument(
    "--data_name",
    type=str,
    default="source_dataset", #change this to the desired filename (no need to include .npz)
...
def process(gt_name: str, image_name: str):
    ...
    if np.sum(gt_data) > 0:  # exclude tiny objects
        """Optional binary thresholding can be added"""
        assert (
            np.max(gt_data) == 1 and np.unique(gt_data).shape[0] >= 1
