...
def calculate_iou(annotated_imgs_directory, label_imgs_directory):
    ...
    for label_img_name in tqdm(label_imgs):
...
excel_file_path = 'results/IoU.xlsx'
...
image_names = list(IOUs.keys())
iou_values = list(IOUs.values())

# Plotting
plt.figure(figsize=(10, 6))
plt.plot(image_names, iou_values, marker='o', linestyle='-')
plt.title('IoU Values for Predicted Images')
plt.xlabel('Image Name')
plt.ylabel('IoU Value')
plt.xticks(image_names)  # Set x ticks to be the image names
plt.grid(True)
plt.show()